...
** (C) 2001, 2003 Ronald Hof, Timm S. M�ller & Per Johansson ;)
**
...
Serial.print(ptd.period); Serial.print(" ");
...
Serial.print((unsigned)buf[1]); Serial.print(" ");
Serial.print((unsigned)buf[2]); Serial.print(" ");
Serial.print((unsigned)buf[3]);
Serial.println("");
#endif
...
if(j != 0)
{
#if 0 /* NOTE: we cannot modify samples down there */
j=tab_funk[j];
c->funkoffset+=j;
...
c->sp=newsp;
c++;
// ptd++; // No need, we will read next pattern channel
}
...
if(mod->state==PT_STATE_PLAYSONG) spos++;
...
{
mod->bpos = j;
pt_dotick(file,mod);	/* apply changes per tick */
}
...
Serial.print(" off ");
Serial.println(s->offset);

Serial.print("SAMP: data for j=="); Serial.print(j>>14);
Serial.print(": ");
Serial.print((int)dat);
Serial.print(" vol ");

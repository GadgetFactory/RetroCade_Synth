...
{
    int i;
    pt_render(&modSDfile, mod, buf, NULL, 2, 16 /* Samples */, 1, 16, 1);
	for (i=0;i<32;i+=2) {
	  unsigned v = buf[i];
	  v += buf[i+1]<<8;
	  v += buf[i]<<16;
...
pt_mod_s *MODPLAYER::pt_init_smallfs(File &file)
{
